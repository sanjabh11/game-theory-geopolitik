...
DO $$
BEGIN
    -- Mental Models Table (if not exists)
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'mental_models') THEN
    ...
    -- User Problem Submissions (if not exists)
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'mental_model_problems') THEN
    ...
    -- Model-Generated Solutions (if not exists)
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'mental_model_solutions') THEN
    ...
    -- User Sessions (if not exists)
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'mental_model_sessions') THEN
    ...
    -- Model Performance Tracking (if not exists)
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'model_performance_tracking') THEN
    ...
    -- Collaborative Problem Solving (if not exists)
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'mental_model_collaborations') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_mental_models_category') THEN
        CREATE INDEX idx_mental_models_category ON mental_models(category);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_mental_models_complexity') THEN
        CREATE INDEX idx_mental_models_complexity ON mental_models(complexity_score);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_mental_model_problems_user_id') THEN
        CREATE INDEX idx_mental_model_problems_user_id ON mental_model_problems(user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_mental_model_problems_domain') THEN
        CREATE INDEX idx_mental_model_problems_domain ON mental_model_problems(domain);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_mental_model_solutions_problem_id') THEN
        CREATE INDEX idx_mental_model_solutions_problem_id ON mental_model_solutions(problem_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_mental_model_solutions_model_id') THEN
        CREATE INDEX idx_mental_model_solutions_model_id ON mental_model_solutions(model_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_mental_model_sessions_user_id') THEN
        CREATE INDEX idx_mental_model_sessions_user_id ON mental_model_sessions(user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_mental_model_sessions_expires_at') THEN
        CREATE INDEX idx_mental_model_sessions_expires_at ON mental_model_sessions(expires_at);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_model_performance_model_id') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mental_models' AND policyname = 'Mental models are publicly readable') THEN
        CREATE POLICY "Mental models are publicly readable" ON mental_models
          FOR SELECT TO public USING (true);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mental_model_problems' AND policyname = 'Users can manage their own problems') THEN
        CREATE POLICY "Users can manage their own problems" ON mental_model_problems
          FOR ALL TO public USING (auth.uid() = user_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mental_model_solutions' AND policyname = 'Users can access solutions for their problems') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mental_model_solutions' AND policyname = 'Users can create solutions for their problems') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mental_model_solutions' AND policyname = 'Users can update their own solutions') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mental_model_sessions' AND policyname = 'Users can manage their own sessions') THEN
        CREATE POLICY "Users can manage their own sessions" ON mental_model_sessions
          FOR ALL TO public USING (auth.uid() = user_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'model_performance_tracking' AND policyname = 'Performance tracking is publicly readable') THEN
        CREATE POLICY "Performance tracking is publicly readable" ON model_performance_tracking
          FOR SELECT TO public USING (true);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'model_performance_tracking' AND policyname = 'Users can add performance tracking') THEN
        CREATE POLICY "Users can add performance tracking" ON model_performance_tracking
          FOR INSERT TO public WITH CHECK (true);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mental_model_collaborations' AND policyname = 'Users can access collaborations they participate in') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_mental_models_updated_at') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM mental_models LIMIT 1) THEN
        ...
        VALUES
          ...
          ('opportunity_cost', 'Opportunity Cost', 'economic', 5, 
