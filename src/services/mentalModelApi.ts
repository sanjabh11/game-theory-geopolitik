...
export class MentalModelApiService {
  ...
  async analyzeProblem(problem: ProblemInput): Promise<ApiResponse<{
    ...
    try {
      // Use Gemini API to analyze the problem
      ...
      return {
        success: true,
        data: {
          recommended_models: response.data.recommendedModels.map(model => ({
            model_id: model.modelId,
            model_name: model.modelName,
            relevance_score: model.relevanceScore,
            rationale: model.explanation
          })),
          problem_analysis: {
            complexity: response.data.structuredData.complexityLevel,
            key_factors: response.data.structuredData.constraints,
            domain_classification: problem.domain,
            uncertainty_level: response.data.structuredData.complexityLevel * 10
          }
        }
      };
    } catch (error) {
      ...
      return this.generateMockProblemAnalysis(problem);
    }
  }
  ...
  async generateSolution(modelId: string, problem: ProblemInput): Promise<ApiResponse<ModelSolution>> {
    try {
      ...
      // Use Gemini API to generate a solution
      const prompt = `
...
The ${model.name} approach involves: ${model.prompt_template.replace('{problem}', 'a problem')}

Provide a comprehensive solution including:
1. Detailed solution text
...
`;
      ...
      try {
        const response = await geminiApi.makeRequest(prompt);
        ...
        return {
          ...
          data: {
            ...
            solution_text: parsedResponse.solution_text,
        ...
        console.warn('API error, using fallback solution:', apiError);
        // Fallback to mock solution
        return this.generateMockSolution(model, problem);
      }
    } catch (error) {
      return {
        success: false,
  ...
  private generateMockProblemAnalysis(problem: ProblemInput): ApiResponse<{
    recommended_models: Array<{
      model_id: string;
      model_name: string;
      relevance_score: number;
    ...
    const recommendations = recommendedModelIds.map((id, index) => {
      ...
      return {
        ...
        rationale: `${model.name} is well-suited for ${detectedDomain} problems because it ${this.getModelRationale(model.id, detectedDomain)}`
      };
    }).filter(Boolean);
    ...
    return {
      ...
      data: {
        ...
        problem_analysis: {
          ...
          key_factors: this.extractKeyFactors(problem.problem_text),
  ...
  private generateMockSolution(model: MentalModel, problem: ProblemInput): ApiResponse<ModelSolution> {
    return {
      success: true,
      data: {
        model_id: model.id,
        model_name: model.name,
        solution_text: this.generateSolutionText(model.id, problem.problem_text),
        confidence_score: Math.floor(70 + Math.random() * 25),
        reasoning: `This solution applies ${model.name} by ${this.getModelApproach(model.id)}`,
        alternatives: this.generateAlternatives(model.id),
        limitations: model.limitations,
        next_steps: this.generateNextSteps(model.id)
      }
    };
  }

  private getModelRationale(modelId: string, domain: string): string {
  ...
  private getModelApproach(modelId: string): string {
  ...
  private extractKeyFactors(problemText: string): string[] {
  ...
  private generateSolutionText(modelId: string, problemText: string): string {
  ...
  private generateAlternatives(modelId: string): string[] {
  ...
  private generateNextSteps(modelId: string): string[] {
    return [
